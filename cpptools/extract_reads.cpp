...
int main(int argc, char* argv[])
{
    ...
    // std::vector<std::fstream> output_reads1, output_reads2, output_barcode, output_iterleaved;
    std::vector<std::fstream> output_barcode, output_iterleaved;
    std::string line;
    ...
    while (getline(clustersf, line))
    {
        ...
        output_iterleaved.push_back(std::move(f4));
        std::string barcode;
        while (pos != line.size())
        {
    ...
    while (getline(reads1f, line1))
    {
        ...
        case 0:
            ...
            if (isSatisfied)
            {
                ...
                output_iterleaved.at(barcode2cluster.at(p1.second)) << reads_seq1 << reads_seq2;
                reads_seq1.clear();
                reads_seq2.clear();
            }
            break;
