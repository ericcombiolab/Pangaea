...
int main(int argc, char* argv[])
{
    ...
    // Support both compressed (.gz) and uncompressed fastq1/fastq2
    std::ifstream fastq1_plain, fastq2_plain;
    igzstream fastq1_gz, fastq2_gz;
    std::istream* fastq1_stream = nullptr;
    std::istream* fastq2_stream = nullptr;
    bool fastq1_is_gz = (fastq1_path.size() > 3 && fastq1_path.substr(fastq1_path.size() - 3) == ".gz");
    bool fastq2_is_gz = (fastq2_path.size() > 3 && fastq2_path.substr(fastq2_path.size() - 3) == ".gz");
    if (fastq1_is_gz) {
        fastq1_gz.open(fastq1_path.c_str());
        fastq1_stream = &fastq1_gz;
    }
    else {
        fastq1_plain.open(fastq1_path.c_str());
        fastq1_stream = &fastq1_plain;
    }
    if (fastq2_is_gz) {
        fastq2_gz.open(fastq2_path.c_str());
        fastq2_stream = &fastq2_gz;
    }
    else {
        fastq2_plain.open(fastq2_path.c_str());
        fastq2_stream = &fastq2_plain;
    }
    ...
    do
    {
        if (std::getline(*fastq1_stream, line1) && std::getline(*fastq2_stream, line2)) {
            read_cnt++;
            read_name[openmp_index] = std::move(getReadname(line1));
            std::getline(*fastq1_stream, line1);
            std::getline(*fastq2_stream, line2);
